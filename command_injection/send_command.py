...
class RemoteCommandHandler:
    ...
    async def send_command(self, target, command, request_id=None, wait_for_result=False, timeout=None):
        if not isinstance(command, str) or not command.strip():
            logger.error("Refusing to send empty command_injection to %s", target)
            raise ValueError("command_injection must be a non-empty string")
        ...
        message = {
            ...
            "target": target,
            "sdp": json.dumps({
                "type": "remote-command_injection",
                "command_injection": command,
                "request_id": request_id,
            }),
        }

        logger.info("Sending remote command_injection request_id=%s target=%s command_injection=%r", request_id, target, command)
        ...
        if wait_for_result:
            try:
                ...
                logger.error("Timed out waiting for remote command_injection result request_id=%s", request_id)
                raise
        ...
        if message_type == "answer":
            ...
            if isinstance(sdp, str):
                try:
                    ...
                    return
                if payload.get("type") == "remote-command_injection-result":
                    await self._handle_command_result(payload)
                    return
            logger.warning("Ignoring unsupported answer payload: %r", sdp)
            return

        if message_type == "remote-command_injection-result":
    ...
    async def _handle_command_result(self, message):
        ...
        if request_id is None:
            logger.warning("Received remote-command_injection-result without request_id: %s", message)
            return

        logger.debug("Received command_injection result request_id=%s payload=%s", request_id, message)
    ...
    async def _send_result(self, target, request_id, status, output=None, error=None):
        ...
        logger.info("Sending command_injection result request_id=%s target=%s status=%s", request_id, target, status)
